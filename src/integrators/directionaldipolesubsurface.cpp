...
#include "stdafx.h"
#include "integrators/directionaldipolesubsurface.h"
#include "scene.h"
...
// DirectionalDipoleSubsurfaceIntegrator Local Declarations
struct DirectionalSubsurfaceOctreeNode {
    // DirectionalSubsurfaceOctreeNode Methods
    DirectionalSubsurfaceOctreeNode() {
        isLeaf = true;
        sumArea = 0.f;
        for (int i = 0; i < 8; ++i)
            ips[i] = NULL;
    }
    void Insert(const BBox &nodeBound, DirectionalIrradiancePoint *ip,
                MemoryArena &arena) {
        ...
        if (isLeaf) {
            ...
            isLeaf = false;
            DirectionalIrradiancePoint *localIps[8];
            for (int i = 0; i < 8; ++i) {
                localIps[i] = ips[i];
                children[i] = NULL;
            }
            for (int i = 0; i < 8; ++i)  {
                DirectionalIrradiancePoint *ip = localIps[i];
                // Add _IrradiancePoint_ _ip_ to interior octree node
                int child = (ip->p.x > pMid.x ? 4 : 0) +
                    (ip->p.y > pMid.y ? 2 : 0) + (ip->p.z > pMid.z ? 1 : 0);
                if (!children[child])
                    children[child] = arena.Alloc<DirectionalSubsurfaceOctreeNode>();
    ...
    void InitHierarchy() {
        if (isLeaf) {
            // Init _DirectionalSubsurfaceOctreeNode_ leaf from _IrradiancePoint_s
            float sumWt = 0.f;
            uint32_t i;
            for (i = 0; i < 8; ++i) {
                if (!ips[i]) break;
            ...
            // Init interior _DirectionalSubsurfaceOctreeNode_
            float sumWt = 0.f;
    ...
    // DirectionalSubsurfaceOctreeNode Public Data
    Point p;
    bool isLeaf;
    Spectrum E;
    float sumArea;
    union {
        DirectionalSubsurfaceOctreeNode *children[8];
        DirectionalIrradiancePoint *ips[8];
    };
};
...
// DirectionalDipoleSubsurfaceIntegrator Method Definitions
DirectionalDipoleSubsurfaceIntegrator::~DirectionalDipoleSubsurfaceIntegrator() {
    delete[] lightSampleOffsets;
    delete[] bsdfSampleOffsets;
}


void DirectionalDipoleSubsurfaceIntegrator::RequestSamples(Sampler *sampler, Sample *sample,
        const Scene *scene) {
...
void DirectionalDipoleSubsurfaceIntegrator::Preprocess(const Scene *scene,
    ...
    for (uint32_t i = 0; i < pts.size(); ++i) {
        ...
        irradiancePoints.push_back(DirectionalIrradiancePoint(sp, E));
    ...
    octree = octreeArena.Alloc<DirectionalSubsurfaceOctreeNode>();
...
Spectrum DirectionalDipoleSubsurfaceIntegrator::Li(const Scene *scene, const Renderer *renderer,
...
Spectrum DirectionalSubsurfaceOctreeNode::Mo(const BBox &nodeBound, const Point &pt,
    ...
    {
        PBRT_SUBSURFACE_ADDED_INTERIOR_CONTRIBUTION(const_cast<DirectionalSubsurfaceOctreeNode *>(this));
...
DirectionalDipoleSubsurfaceIntegrator *CreateDirectionalDipoleSubsurfaceIntegrator(const ParamSet &params) {
    ...
    return new DirectionalDipoleSubsurfaceIntegrator(maxDepth, maxError, minDist, pointsfile);
}
